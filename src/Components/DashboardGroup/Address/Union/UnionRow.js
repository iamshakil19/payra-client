...
const UnionRow = ({ union, index, refetch, setUnionDeleteData, setUnionEditData }) => {
    ...
    return (
        <tr className='hover'>
            ...
            <td className='bangla-font p-2'>
                <label onClick={() => { setUnionEditData(union) }} htmlFor="edit-union-modal" className=' w-8 h-8 text-center bg-indigo-200 text-xl text-indigo-800 border border-indigo-300 rounded-md cursor-pointer flex justify-center items-center hover:bg-indigo-500 hover:text-white hover:border-indigo-600 transition-all ease-in-out duration-200'><span className=''><MdEdit /></span></label>
            </td>
            <td className='p-2'>
